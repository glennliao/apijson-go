...
func NewSqlExecutor(ctx context.Context, tableName string, accessVerify bool) (*SqlExecutor, error) {
	...
	var columns []string
	fields, err := g.DB().TableFields(ctx, tableName)
	if err != nil {
		return nil, err
	}
...
func (e *SqlExecutor) ParseCtrl(ctrl g.Map) error {
	...
	for k, v := range ctrl {
		...
		str := strings.ReplaceAll(gconv.String(v), ";", Separator)

		split := strings.Split(str, ",")
		for i, item := range split {
			split[i] = exp.ReplaceAllStringFunc(item, config.ToDbField)
		}

		str = strings.Join(split, Separator)

		switch k {

		case "@order":
			order := strings.ReplaceAll(str, "-", DESC)
